    ...
    while True:
        ...
        if opcion == "1":
            while True:
                ...
                while True:
                    fecha_reservacion = input("Ingrese la fecha de la reservación (DD/MM/YYYY o DD-MM-AAAA o YYYY-MM-DD) o 'CANCELAR' para salir: ").strip()
            ...
            while True:
                ...
                fecha_inicio = input("Ingrese la fecha de inicio (DD/MM/YYYY) o 'CANCELAR' para salir: ").strip()
                if fecha_inicio.upper() == "CANCELAR":
                    break
                fecha_fin = input("Ingrese la fecha de fin (DD/MM/YYYY) o 'CANCELAR' para salir: ").strip()
            ...
            while True:
                ...
                fecha_consulta = input("Ingrese la fecha a consultar (DD/MM/YYYY) o ENTER para fecha actual, o 'CANCELAR' para salir: ").strip()
                ...
                print(f"\n{'-'*30} REPORTE DE RESERVACIONES PARA EL DÍA ({fecha_obj.strftime('%d/%m/%Y')}) {'-'* 38}")
            ...
            while True:
                ...
                if nombre.upper() == "CANCELAR":
                    break
                if not nombre:
                    print("El nombre no pueden estar vacío.")
                    continue
            ...
            while True:
                with sqlite3.connect("reservaciones.db") as conn:
                    ...
                    fecha_inicio = input("Ingresa la fecha de inicio (DD/MM/AAAA) o 'CANCELAR': ").strip()
                    if fecha_inicio.upper() == "CANCELAR":
                        break
                    fecha_fin = input("Ingresa la fecha final (DD/MM/AAAA) o 'CANCELAR': ").strip()
                    if fecha_fin.upper() == "CANCELAR":
                        break
                    try:
                        fecha_i = datetime.datetime.strptime(fecha_inicio, "%d/%m/%Y").date()
                        fecha_f = datetime.datetime.strptime(fecha_fin, "%d/%m/%Y").date()
                    except ValueError:
                        print("Formato inválido. Usa exclusivamente el formato DD/MM/AAAA.")
                        continue
                    ...
                    for r in reservas_filtradas:
                        print(f"Folio: {r[0]} | Cliente: {r[1]} | Sala: {r[2]} | Fecha: {parse_fecha(r[3]).strftime('%d/%m/%Y')} | Evento: {r[4]}")
